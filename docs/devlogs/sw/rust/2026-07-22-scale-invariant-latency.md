...
At 1,000 keys on ARMv8.4-A Firestorm (12 MB L2 cache), everything fits in cache. CoordMap2 get costs 21.5 ns. HashMap get costs 24.4 ns. The difference is 2.9 ns — statistically significant but not decisive. At this scale, the benchmark measures conversion overhead: str-to-CoordKey (21 ns) vs SipHash-2-4 (the balance of HashMap's latency). Both are computation-bound, memory is not the bottleneck.

The takeaway at 1k: str-to-Coord is competitive with SipHash. The bridge is viable.

## The 10M story

At 10,000,000 operations on 65,536 keys (cycling 153 times), the picture changes. CoordMap2 remains flat at 21.5 ns. HashMap rises to 23.8 ns — a 19% increase from its 10k baseline of 20.0 ns.
...
```
  ...
  CoordMap2    22.0 ── 21.4 ── 21.5     flat (±0.6 ns over 1000x scale change)
...
CoordMap2 insert at 233 ms for 65,536 keys is almost entirely the 119 MB `alloc_zeroed` call for CoordSpace2 construction. The actual slot write is ~22 ns per key. This is visible in the benchmark because each criterion iteration creates a fresh CoordMap2, allocating 119 MB every time. A real application would allocate once and reuse.
...
For TagmaMap, the practical implication is that the performance observed at 1k is the performance at any scale. No tuning, no capacity planning, no load factor monitoring. The 22 ns get latency at 1k is the same 22 ns at 10M — and will be the same at 1B, limited only by the DRAM bandwidth of the array.
