...
The Sky130 standard cell flow is set up under `hw/openroad/` with ORFS, the official OpenROAD Flow Scripts image. The registered demo top is the design target so the report maps one to one to the FPGA demo; the pure decoder gate count is reported separately with `yosys stat -liberty` against the Sky130 cell library. The flow runs in the `hw` CI job on the x86 runner and uploads the reports as an artifact.

Local findings: the ORFS image is x86_64 and runs on Apple Silicon under Rosetta and on x86. Two flow bugs were fixed during derisking: a virtual clock definition (create_clock without a source port) left the design without a clock tree, and the default-on kepler-formal LEC step crashes with an illegal instruction on both platforms, so it is disabled via `LEC_CHECK = 0` in the design config. The full flow completes on both platforms. The ORFS image ships the Sky130 PDK, so no download is needed.
