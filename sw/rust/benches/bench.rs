...
fn mixed_workload(count: usize) -> Vec<MixedOp> {
    ...
    for _ in 0..count {
        let coord = Coord::new(rng.gen_range(0..N as u16)).unwrap();
...
fn bench_cs2_get_all(c: &mut Criterion) {
    ...
    c.bench_function("CoordSpace2/get/all_11172", |b| {
        b.iter(|| {
            for &coord in &coords {
                black_box(black_box(&space).at_path(&tagma_core::CoordPath::new([coord, coord])));
            }
        })
    });
}
...
fn bench_tagma_get_single(c: &mut Criterion) {
    let coord = Coord::new(5000).unwrap();
    let mut space = tagma_core::CoordSpace::new();
...
fn bench_spatial_cs2_prefix_scan(c: &mut Criterion) {
    ...
    for p in 0u16..1000 {
        for s in 0u16..100 {
            let path = tagma_core::CoordPath::new([Coord::new(p).unwrap(), Coord::new(s).unwrap()]);
    ...
    group.bench_function("CoordSpaceN2", |b| {
        let prefix_path = vec![Coord::new(42).unwrap()];
        b.iter(|| {
            let count = cs2
    ...
    for p in 0u16..10000 {
        for s in 0u16..1000 {
            let path = tagma_core::CoordPath::new([
                Coord::new((p * 22) % (Coord::N_VALID as u16)).unwrap(),
                Coord::new((s * 587 + p) % (Coord::N_VALID as u16)).unwrap(),
            ]);
            paths.push((path, p, s));
            cs2.place_path(&path, (p * 1000 + s).into());
            hm.insert(
                (
                    (p * 22) % (Coord::N_VALID as u16),
                    (s * 587 + p) % (Coord::N_VALID as u16),
                ),
                (p * 1000 + s).into(),
            );
        }
    ...
    group.bench_function("HashMap", |b| {
        b.iter(|| {
            let count = hm
                .iter()
                .filter(|((p, s), _)| {
                    Coord::new(*p).unwrap().to_axes().0 == 3
                        && Coord::new(*s).unwrap().to_axes().1 == 7
                })
                .count();
            black_box(count);
        })
...
fn bench_cs2_nonexistent_prefix(c: &mut Criterion) {
    ...
    for p in 0u16..10000 {
        for s in 0u16..1000 {
            ...
            cs2.place_path(&path, (p * 1000 + s).into());
            hm.insert(
                (p, (s * 587 + p) % (Coord::N_VALID as u16)),
                (p * 1000 + s).into(),
            );
        }
    }
    ...
    group.bench_function("CoordSpaceN2", |b| {
        let missing = vec![Coord::new(11111).unwrap()];
...
fn bench_coordset_spatial_query(c: &mut Criterion) {
    ...
    let initial_sets: Vec<CoordSet> = (0..19u8)
        .map(|init| {
            ...
            for m in 0..21u8 {
                for f in 0..28u8 {
                    set.insert(Coord::from_axes(init, m, f).unwrap());
                }
            }
            set
        })
    ...
    let medial_sets: Vec<CoordSet> = (0..21u8)
        .map(|med| {
            ...
            for i in 0..19u8 {
                for f in 0..28u8 {
                    set.insert(Coord::from_axes(i, med, f).unwrap());
                }
            }
            set
        })
...
//
// Overhead vs raw CoordPath:
//   raw_path_get_3x (access 6 coords directly)    319 ps    baseline
//   cube_axis_3x (extract 3 axes via CoordCube)    319 ps    zero-cost (identical)
//   cube_from_path (wrap CoordPath in CoordCube)   958 ps    one-time construction
//
// Proximity generation (D=2, R=1, 2D):
//   r=0 (1 path)      16.5 ns    60.6 Melem/s
//   r=1 (9 paths)     86.6 ns   103.9 Melem/s
//   r=2 (25 paths)   126.4 ns   197.8 Melem/s
//   r=3 (49 paths)   184.6 ns   265.5 Melem/s
//   r=5 (121 paths)  331.4 ns   365.1 Melem/s
//
// Bounding box:
//   N=2, 100x100 (10,201 paths)   14.28 µs   714 Melem/s
//   N=2, 10x10 (100 paths)          300 ns   333 Melem/s
//   N=4, 4^4 (256 paths)            868 ns   295 Melem/s
...
// ===========================================================================
// CoordCube 2-channel benchmarks: dimensional scaling, distance metrics, map queries
// ===========================================================================
...
fn bench_coordcube_path_baseline(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Spatial/cubebaseline");

    group.bench_function("manual_9_path_loop", |b| {
        b.iter(|| {
            let mut count = 0usize;
            for d0 in -1i16..=1i16 {
                let c0 = Coord::new((5000i16 + d0) as u16).unwrap();
                for d1 in -1i16..=1i16 {
                    let c1 = Coord::new((5000i16 + d1) as u16).unwrap();
                    black_box(CoordPath::new([c0, c1]));
                    count += 1;
                }
            }
...
fn bench_coordcube_compound(c: &mut Criterion) {
    ...
    // Pre-compute axis-indexed CoordSets (same as bench_coordset_spatial_query)
    let initial_sets: Vec<CoordSet> = (0..19u8)
        .map(|init| {
            let mut set = CoordSet::new();
            for m in 0..21u8 {
    ...
    let mut group = c.benchmark_group("Spatial/cubecompound");

    // Phase 1: proximity gen only
    group.bench_function("proximity_r1_gen", |b| {
        b.iter(|| {
    ...
    // Phase 1+2: proximity + CoordSet filter
    group.bench_function("proximity_r1_then_coordset_filter", |b| {
        let set3 = &initial_sets[3];
        b.iter(|| {
            let count = cube
                .proximity(1)
                .filter(|p| set3.contains(p.coords()[0]))
                .count();
            black_box(count);
        })
    });

    // Comparison: CoordSet only (no proximity)
    group.bench_function("coordset_only_axis_3_5", |b| {
        let set3 = &initial_sets[3];
        let set5 = &initial_sets[5];
        b.iter(|| {
            let result = set3.intersection(set5);
            black_box(result.len());
        })
...
// N_scaling/get  (single lookup, ARMv8.4-A Firestorm, fresh 2026-07)
//   N=1   CoordSpace      0.38 ns   space 10^4  (inline stack array, no alloc)
//   N=2   CoordSpace2     0.38 ns   space 10^8  (dense heap)
//   N=2   CoordSpaceN2    0.87 ns   space 10^8  (tree)
//   N=3   CoordSpaceN3    2.62 ns   space 10^12
//   N=3   CoordSpaceM3    0.40 ns   space 10^12 (mmap dense, 6.6x faster)
//   N=6   CoordSpaceN6    5.44 ns   space 10^24
//   N=12  CoordSpaceN12   19.0  ns   space 10^67
//   N=19  CoordSpaceN19   47.8  ns   space 10^77 (SHA-256 scale)
fn bench_n_scaling_get(c: &mut Criterion) {
    let path6 =
        tagma_core::CoordPath::<6>::new(core::array::from_fn(|i| Coord::new(i as u16).unwrap()));
    let path12 =
        tagma_core::CoordPath::<12>::new(core::array::from_fn(|i| Coord::new(i as u16).unwrap()));
    let path19 =
        tagma_core::CoordPath::<19>::new(core::array::from_fn(|i| Coord::new(i as u16).unwrap()));
    let path2 = tagma_core::CoordPath::<2>::new([Coord::new(0).unwrap(), Coord::new(1).unwrap()]);
    ...
    {
        let mut cs = tagma_core::CoordSpace::new();
        cs.place(Coord::new(0).unwrap(), 42u64);
        let mut group = c.benchmark_group("N_scaling/get/N=1");
        group.bench_function("CoordSpace", |b| {
            b.iter(|| black_box(cs.at(&Coord::new(0).unwrap())))
        });
        group.finish();
    }
    {
...
// CoordSpace2/insert/1000           120 µs  (dense array, fresh 2026-07)
...
// CoordSpaceN2/get/1000             46.6 µs
// CoordSpace2/get/1000              31.0 µs  (dense array, fresh 2026-07)
fn bench_cm2_get_1000(c: &mut Criterion) {
    let map_n2 = {
        ...
        for i in 0u16..100 {
            for j in 0u16..10 {
                let path =
                    tagma_core::CoordPath::new([Coord::new(i).unwrap(), Coord::new(j).unwrap()]);
...
criterion_group!(
    ...
    targets = bench_tagma_insert_all, bench_std_insert_all,
              ...
              bench_cs2_insert_all, bench_csn2_insert_all
);
criterion_group!(
    name = lookup;
    config = Criterion::default();
    targets = bench_tagma_get_all, bench_std_get_all,
              ...
              bench_cs2_get_all, bench_csn2_get_all
);
criterion_group!(
    name = mutate;
    config = Criterion::default();
...
//            vs std HashMap for string-key map workloads
// ===========================================================================
//
...
fn map_short_keys(count: usize) -> Vec<String> {
    (0..count)
        .map(|i| format!("k{:05}", i % 100_000)) // "k00000" .. "k99999" (6 bytes each)
        .collect()
}

fn map_medium_keys(count: usize) -> Vec<String> {
    (0..count)
...
fn bench_map_single_insert_hashmap(c: &mut Criterion) {
    ...
    c.bench_function("kv/hashmap/insert/single", |b| {
        let mut map: std::collections::HashMap<String, Box<[u8]>> =
            std::collections::HashMap::new();
        b.iter(|| {
            black_box(map.insert(key.to_string(), map_boxed(&map_value())));
        })
    });
}
...
fn bench_map_batch_insert_static_short(c: &mut Criterion) {
    let keys: Vec<(CoordPath<2>, Box<[u8]>)> = map_short_keys(1000)
        .iter()
        .map(|k| {
            let path = Prefix::<2>.generate(k).unwrap();
            let cp = CoordPath::new([path[0], path[1]]);
            (cp, map_boxed(&map_value()))
        })
        .collect();
    ...
    c.bench_function("kv/static/insert/short_1k", |b| {
        ...
        b.iter(|| {
            for (cp, val) in &keys {
                black_box(space.place_path(cp, map_boxed(val)));
            }
            black_box(&space);
        })
    });
...
fn bench_map_batch_insert_dyn_short(c: &mut Criterion) {
    let keys: Vec<(Vec<tagma_core::Coord>, Box<[u8]>)> = map_short_keys(1000)
        .iter()
        .map(|k| {
            let path = ByteWise.generate(k).unwrap();
            (path, map_boxed(&map_value()))
        })
        .collect();
    ...
    c.bench_function("kv/dynamic/insert/short_1k", |b| {
        ...
        b.iter(|| {
            for (path, val) in &keys {
                black_box(space.place(path, map_boxed(val)));
            }
            black_box(&space);
        })
    });
...
fn bench_map_batch_insert_hashmap_short(c: &mut Criterion) {
    let keys: Vec<(String, Box<[u8]>)> = map_short_keys(1000)
        .iter()
    ...
    c.bench_function("kv/hashmap/insert/short_1k", |b| {
        ...
        b.iter(|| {
            for (k, val) in &keys {
                black_box(map.insert(k.clone(), map_boxed(val)));
            }
            black_box(&map);
        })
    });
...
fn bench_map_batch_insert_static_medium(c: &mut Criterion) {
    let keys: Vec<(CoordPath<2>, Box<[u8]>)> = map_medium_keys(1000)
        .iter()
        .map(|k| {
            let path = Prefix::<2>.generate(k).unwrap();
            let cp = CoordPath::new([path[0], path[1]]);
...
fn bench_map_batch_insert_dyn_medium(c: &mut Criterion) {
    let keys: Vec<(Vec<tagma_core::Coord>, Box<[u8]>)> = map_medium_keys(1000)
        .iter()
        .map(|k| {
            let path = ByteWise.generate(k).unwrap();
            (path, map_boxed(&map_value()))
...
fn bench_map_batch_insert_hashmap_medium(c: &mut Criterion) {
    let keys: Vec<(String, Box<[u8]>)> = map_medium_keys(1000)
        .iter()
        .map(|k| (k.clone(), map_boxed(&map_value())))
        .collect();
...
fn bench_map_batch_get_all(c: &mut Criterion) {
    ...
    // HashMap: pre-populate
    {
        let mut map: std::collections::HashMap<String, Box<[u8]>> =
            std::collections::HashMap::new();
        for k in &short_keys {
            map.insert(k.clone(), map_boxed(&map_value()));
        }
...
use tagma_map::coord_map_n::CoordMapN;
use tagma_map::{CoordKey, CoordMap, CoordMap2, CoordMapKey, DynCoordMap};

fn map_2byte_keys(count: usize) -> Vec<String> {
...
fn map_2byte_all() -> Vec<CoordKey<2>> {
...
fn bench_map_wrapper_single_insert(c: &mut Criterion) {
    ...
    group.bench_function("HashMap<String>", |b| {
        ...
        b.iter(|| {
            black_box(map.insert("k0".to_string(), map_value()));
        })
    });
...
fn bench_map_wrapper_single_get(c: &mut Criterion) {
    ...
    group.bench_function("HashMap<String>", |b| {
        let mut map: std::collections::HashMap<String, Vec<u8>> = std::collections::HashMap::new();
        map.insert("k0".to_string(), map_value());
        b.iter(|| {
...
fn bench_map_wrapper_batch_get(c: &mut Criterion) {
    let short_keys = map_short_keys(10_000);
    let byte2_keys = map_2byte_keys(676);
    let val = map_value();
...
// map_workload(name, unique_keys, cycles):
//   get/contains: unique_keys × cycles total operations
//   insert: unique_keys unique inserts
//
// Key count notes:
//   - DynCoordMap/HashMap: uses map_short_keys (up to 100k unique 6-char keys)
...
fn map_workload(c: &mut Criterion, scale: &str, n: usize, cycles: usize) {
    let keys = map_short_keys(n);
...
fn map_workload_10k(c: &mut Criterion) {
    map_workload(c, "10k", 10_000, 1);
}
fn map_workload_1m(c: &mut Criterion) {
    map_workload(c, "1M", 65_536, 15);
}
fn map_workload_10m(c: &mut Criterion) {
    map_workload(c, "10M", 65_536, 153);
}
...
criterion_group!(
    ...
    targets = bench_map_single_insert_static, bench_map_single_insert_dyn, bench_map_single_insert_hashmap,
              ...
              map_workload_10k, map_workload_1m, map_workload_10m,
);
...
fn bench_cs6_sparse_1k(c: &mut Criterion) {
    ...
    let mk = |v: u16| Coord::new(v).unwrap();
    ...
    group.bench_function("HashMap", |b| {
        b.iter(|| {
            let count = hm
                .iter()
                .filter(|(hk, _)| {
                    Coord::new(hk[2]).unwrap().to_axes().0 == 2
                        && Coord::new(hk[4]).unwrap().to_axes().1 == 3
                })
                .count();
            black_box(count);
        })
