...
use crate::types::{Attestation, Decision, Epoch, Event, Path, SignedEvidence};
...
/// signed evidence proving origin (non-repudiation).
#[derive(Clone, Debug)]
pub struct RouteUpdate {
    /// Audit entry committing to the record.
    pub event: Event,
    /// Signed evidence binding the record to its path and epoch.
...
/// the current epoch and the sealed record verifies. The record is then
/// committed to the audit log, and the channel signs evidence binding the
/// record to its path and epoch; the signed evidence is committed to the
/// audit log as the non-repudiation receipt. Returns `None` on any rejected
/// step.
pub fn route_update(
    stack: &mut SecStack,
    att: &Attestation,
    ...
    // Non-repudiation: bind the record to its path and epoch, sign with the
    // channel, and commit the signed evidence to the audit log (receipts are
    // appended by the caller per spec/tagma-sec.md).
    ...
    stack.audit.append(&signed.evidence, epoch);

    Some(RouteUpdate { event, signed })
}
